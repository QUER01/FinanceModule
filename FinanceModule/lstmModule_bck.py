...
def designModel(train_X, return_sequence, n_input, features):
    # design network
    model = Sequential()
    model.add(LSTM(units=50, return_sequences=True, input_shape=(n_input, features)))
...
for column in df_stockHistoryCleaned:
    ...
    for i in range(0, 300):
        ...
        # scaled, scaler = rescaleData(data = df_stockHistoryCleanedColumn)
        # reframe the dataset to an unsupervised learning dataset
        train = series_to_supervised(df_stockHistoryCleanedColumn, n_input, n_output)

        scaler = MinMaxScaler()
        scaler.fit(df_stockHistoryCleanedColumn)
        train = scaler.transform(train)

        train_X, train_y, test_X, test_y = splitTrainTestXY(train=train, test=train)
        # split the dataset into train and test sets
        train_X, train_y = train[:, :-1], train[:, -1]
        ...
        if i == 0:
            ...
            # Design the model
            newModel = designModel(train_X, return_sequence=False, n_input=n_input, features=features)
            # fit network
            history = newModel.fit(train_X, train_y, epochs=10, batch_size=72, verbose=2,
                                   callbacks=[EarlyStopping(monitor='val_loss', patience=10)], shuffle=False)
        ...
        X = train_last_values[0:train_last_values.shape[0],
            0:train_last_values.shape[1] - 1]  # getting last row, gettin all -1 values from last row
        X = X.reshape((0, train_X.shape[1], 1))
        ...
        forecast = forecast_lstm(model=newModel, X=X, n_batch=1)

        forecasts_for_future = scaler.inverse_transform(np.array(forecast).reshape(-1, 1))
